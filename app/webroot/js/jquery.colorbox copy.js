	...
	cboxPublic.resize = function (object) {
		...
		function setPosition (s) {
			cboxPublic.position(s, function(){
				...
				if(settings.iframe){
					$loaded.append("<iframe id='cboxIframe'" + (settings.scrolling ? " " : "scrolling='no'") + " name='iframe_"+new Date().getTime()+"' allowtransparency=true frameborder=0 src='"+(settings.href || element.href)+"' />");
				}
