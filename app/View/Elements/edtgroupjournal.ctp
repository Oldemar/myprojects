										<?php if ((isset($ischk) && $ischk) || (isset($isedt) && $isedt)) { ?>
										<div id="usrgrpdel">
										<?php
											if ($isedt) {
												$checked = false;
												foreach ($usrjrnl as $checkuser):
													if ($checkuser['Journalperm']['tablename_id'] == 10 && 
													$checkuser['Journalperm']['id_value'] == $grp['Group']['id']) 
														{
														$checked=true;
														break;
														}
											...
											echo $this->Form->checkbox('gsharing.'.$grp['Group']['id'], array(
																'checked'=>$checked
																)); 
										?>
										</div>
